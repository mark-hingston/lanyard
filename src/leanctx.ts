...
const leanCtxHookBash = (subcommand: string) =>
  `command -v lean-ctx >/dev/null 2>&1 || { echo '[skip] lean-ctx not on PATH, skipping hook' >&2; exit 0; }; lean-ctx ${subcommand}`;
const leanCtxHookPowershell = (subcommand: string) =>
  `if (-not (Get-Command lean-ctx -ErrorAction SilentlyContinue)) { Write-Warning 'lean-ctx not on PATH, skipping hook'; exit 0 }; lean-ctx ${subcommand}`;
